...
O Detector por Produto Interno (DPI) nada mais é que um classificador construído a partir de um treinamento de imagens previamente selecionadas. Esses classificadores são considerados rápidos por realizarem a classificação através de uma operação simples de produto interno, além de minimizarem o erro quadrático \cite{waldir}. Com o intuito de aumentar sua robustez a pequenas rotações, Sabino  \cite{waldir} acrescentou a técnica de Análise de Componentes Principais, {\color{red}vista na seção XY} (do inglês, Principal Component Analysis - PCA, {\color{red}se eu já tiver falado anteriormente não vai precisar desse parêntese}).
...
O termo \textit{eigenfaces} refere-se a um algoritmo que faz uso do padrão comum existente nas faces para representá-los em um subespaço de dimensão inferior ao original, ou seja, a partir de diversas faces com pequenas inclinações podemos gerar um único "template" que corresponde a representação de todos.

Matematicamente podemos obter as \textit{eigenfaces}
