...
As demais etapas não há modificações no sentido de paralelização, pois tratam-se de trechos sequenciais. De um modo geral uma contribuição é tornar o algoritmo paralelizáveis em todos os pontos possíveis, pois o desenvolvimento \textit{multi-threads} consegue extrair o máximo de eficiência dos processadores \textit{multicores}. Uma alternativa para isso seria a paralelização usando a API OpenMP e a utilização da biblioteca OpenCV (\textit{Open Source Computer Vision Library}) de modo a facilitar o trabalho. Outra contribuição seria a aplicação do classificador em uma plataforma móvel, uma vez que a mesma só foi desenvolvida para plataformas do tipo \textit{desktop}.
...
Outra métrica de desempenho para se medir o ganho em ambientes paralelos é a \textbf{eficiência} que é determinada pela fração entre o \textit{Speedup} de \textit{p} processadores sobre \textit{p}, conforme pode ser visto na equação .

\begin{equation} \label{eq_speedup}
efici\^e nci{a_{(p)}} = \frac{{Speedu{p_{(p)}}}}{p}
\end{equation}
