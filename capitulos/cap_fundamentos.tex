...
Antes de apresentarmos a utilização de Detectores por Produto Interno aplicado a Análise de Componentes Principais precisamos fazer algumas definições. Entende-se por "Análise de Componentes Principais", também conhecida como  Transformada de \textit{Karhunen-Loève} \cite{schurmann1996pattern}, o procedimento matemático que faz uso de transformadas para a redução de dimensionalidade de um conjunto de variáveis. A ideia central dessa técnica é obter um pequeno número de componentes principais (combinações lineares) de um conjunto inicial de variáveis, onde podemos encontrar o maior volume de informações possíveis. Dessa maneira, utilizaremos essa técnica no projeto dos classificadores com o intuito de aumentar sua robustez.
