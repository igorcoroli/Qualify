...
Com o intuito de aumentar a robustez do detector por produto interno (IPD) a pequenas rotações, utilizou-se a técnica de análise de componentes principais (do inglês, \textit{principal component analysis} - PCA) \cite{waldir}. A técnica do PCA é utilizada, entre outras coisas, no problema de redução de dimensionalidade. Essa técnica baseia-se na extração de um número desejado de componentes principais de um vetor multidimensional, de modo que ao final da operação tenhamos um vetor cuja dimensão é inferior à original e sua variância seja maximizada \cite{waldir}.

\subsection*{\textit{Eigenpoints}}

O termo \textit{eigenpoints} refere-se a um método que faz uso de PCA para a detecção de pontos fiduciais\footnote{Características salientes comum as faces, como canto da boca, olho, nariz, etc.} (do inglês, \textit{fiducial point}) em imagens. Tendo em vista a similaridade dos pontos fiduciais humanos bem como a localização destes, utiliza-se essa técnica na redução de dimensionalidade tendo em vista que as

para representá-los em um subespaço de dimensão inferior ou igual ao original, assim, podemos utilizar \textit{eigenpoints} para encontrarmos os pontos fiduciais (do inglês, \textit{fiducial point}) de um conjunto de faces, por exemplo. \textit{Eigenpoints} constitui-se um caso específico de \textit{eigenfaces}, porém, ao invés de utilizarmos faces utilizamos somente blocos da região de interesse (ponto fiducial desejado), conforme pode ser visto na Figura \ref{fig:eigenfacesEx} e \ref{fig:eigenpointsEx}. Ao combinarmos essa técnica ao classificador IPD temos como resultado um sistema mais robusto a pequenas rotações \cite{waldir}.
...
Há dois principais modelos de arquitetura paralela, os que fazem uso de memória compartilhada e os que fazem uso de memória distribuída . Os modelos de memória compartilhada utilizam um único espaço de endereçamento de memória que pode ser acessada por todos os processadores em contrapartida os modelos com memória distribuída  não há um espaço de endereçamento global, ou seja, cada processador possui sua própria memória. No que tange a programação existem alguns modelos, porém suas abordagens tornaram-se predominante na HPC, o OpenMP (do inglês, \textit{Open Multi-Processing}) para memória compartilhada e MPI (do inglês, \textit{Message Passing Interface}) para memória distribuída. Isso nos permite definir os modelos paralelos clássicos ou puros \cite{diaz2012survey}. Salienta-se também que com a evolução das CPUs e GPUs hoje existem modelos híbridos.
