...
Conforme foi definida na seção {\color{red}seção XX (seção que fala sobre PCA}) o cálculo do PCA é dado pela equação \eqref{eq_autovalores} e levando-se em consideração todo o conceito de IPD, apresentado na seção \ref{sec_IPD}, supondo a representação de uma variável aleatória que pode ser classificadas nas classes {${A},\,{B},\,...,\,{Z}$} onde desejamos detectar os padrões referentes a classe A. 
